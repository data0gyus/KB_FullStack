...
//        user.setRole(rs.getString("ROLE"));       // 역할
    ...
    public int create(UserVO user) throws SQLException {
        ...
        try (PreparedStatement ptmt = conn.prepareStatement(USER_INSERT)) {

            // 파라미터 바인딩
            ptmt.setString(1, user.getId());        // 사용자 ID
            ptmt.setString(2, user.getPassword());  // 비밀번호
            ptmt.setString(3, user.getName());      // 이름
            ptmt.setString(4, user.getRole());      // 역할
            return ptmt.executeUpdate();            // 쿼리 실행 및 영향받은 행 수 반환
        }
    }
    ...
    public Optional<UserVO> get(String id) throws SQLException {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(USER_GET)) {
            ...
            try(ResultSet rs = pstmt.executeQuery()) {       // 쿼리 실행 및 결과셋 획득
                if(rs.next()) {                               // 결과가 있으면 : id == PK == 조회 성공 시 1행만 존재
