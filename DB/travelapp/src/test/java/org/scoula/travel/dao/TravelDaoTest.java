...
class TravelDaoTest {
    ...
    void testGetTravels() {
        ...
        // 페이징 결과 검증
        assertNotNull(travels, "페이징된 여행지 목록 null임");                 // 참조하는 객체(List 반환 받음)
        assertFalse(travels.isEmpty(), "페이징된 여행지 목록 없음");           // List에 데이터 있음
        assertTrue(travels.size() <= 10, "페이지당 최대 10개 초과"); // 10개 이하 조회
